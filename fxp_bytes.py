...
def serialize_price(x: float) -> bytes:
    ...
    serialize_price(9006104071832581.0)  # that's 0504030201ff3f43 in hex on a
    little-endian ieee754 machine
...
def marshal_message(quote_sequence) -> bytes:
    ...
    b = marshal_message([{'timestamp': datetime(2006,1,2), 'cross': 'GBP/USD',
'price': 1.22041}, \
                             {'timestamp': datetime(2006,1,1), 'cross': 'USD/JPY',
'price': 108.2755}])
    len(b)  # each record is 32 bytes, so 2 of those in this test
    64
     b[:32]  # first record is first 32 bytes
    b'\\x00\\x04\\tT\\xdd5@\\x00GBPUSD\\xbba\\xdb\\xa2\\xcc\\x86\\xf3?\\x00\\x00\\
x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00'
     b[32:]  # second record is next 32 bytes (all sent together in the UDP
datagram)
    b'\\x00\\x04\\t@\\xbf]\\xe0\\x00USDJPY\\x12\\x83\\xc0\\xca\\xa1\\x11[@\\x00\\
x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00\\x00'
    :param quote_sequence: list of quote structures ('cross' and 'price', may also
have 'timestamp')
